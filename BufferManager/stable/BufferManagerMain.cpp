...
static void* scan(void *arg) {
   ...
   while (!stop) {
      ...
      for (unsigned page=start; page<start+10; page++) {
         ...
         //BufferFrame bf = bm->getPage(page, false);
	     // ********************************************************************                  
...
static void* readWrite(void *arg) {
   ...
   for (unsigned i=0; i<100000/threadCount; i++) {
      ...
      //************************************************************************
      //BufferFrame bf = bm->getPage(randomPage(threadNum), isWrite);
...
int main(int argc, char** argv) {
   if (argc==5) {
      ...
      cerr << "usage: " << argv[0] << " <file> <pagesOnDisk> <pagesInRAM>" <<
      " <threads>" << endl;
      exit(1);
   }
   ...
   for (unsigned i=0; i<pagesOnDisk; i++) {
      ...
      // ***********************************************************************
	  // BufferFrame bf = bm->getPage(i, true);      
   ...
   for (unsigned i=0; i<pagesOnDisk; i++) {
      ...
      // ***********************************************************************
      // BufferFrame bf = bm->getPage(i,false);
   ...
   if (totalCount==totalCountOnDisk) {
      ...
      cerr << "error: expected " << totalCount << " but got " 
           << totalCountOnDisk << endl;
      delete bm;
      return 1;
   }
}
